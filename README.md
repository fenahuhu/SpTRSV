...
(3) Type ```make``` to compile the ./test_sptrsv.
...
(4) Type ```make``` to compile the ./test_sptrsv.
...
* To verify the build was successful, a small test can be done: 
	* ```cd``` into where ```./test_sptrsv``` is. 
	* run ```./test_sptrsv -n 1 -rhs 1 -forward -mtx ./ash85/ash85.mtx ``` to test SpTRSV on a input matrix ash85.mtx on a single GPU. If the test was successful, it should output the test pass and run time without any error message.

* To simply the test, using "run.sh" script after setup the MPI execution environment.
...
* ##### if use unified version  
 	...
 	```./test_sptrsv -n [#gpu] -k [#task] -rhs 1 -forward -mtx [A.mtx]```
...
* ##### if use nvshmem version
  ...
  ```mpirun -n [#gpu] -ppn [#gpu] ./test_sptrsv -n 1 -k [#task] -mtx [A.mtx]```
    * ```[#gpu]```: The number of GPU(s) will be applied 
    * ```[#task]```: The number of tasks per GPUs
    * ```[A.mtx]```: It will load input matrix from the given path
